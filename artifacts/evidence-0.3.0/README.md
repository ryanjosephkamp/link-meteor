...
The functional suites used a new task-owned profile. Its optional grants (tabs, the local fixture origin and bookmarks) were requested by the product itself, each with an active user gesture, in a visible Chrome for Testing window. The project owner confirmed clicking Allow on all three native prompts. The tabs request resolved after 77 seconds; the site and bookmark requests resolved 1.3 and 1.2 seconds after they appeared.
