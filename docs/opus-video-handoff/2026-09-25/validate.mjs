// Handoff-only validation: no extension, permission, personal profile or Copy tests.
...
const prompt=await readFile(resolve(dir,'OPUS_PROMPT.txt'),'utf8');
...
for(const value of [verification.liveMain,verification.productSource,verification.build.sha256])assert.ok(md.includes(value));
...
let browser;
try{
 browser=await chromium.launch({headless:true,env:{...process.env,TMPDIR:resolve(root,'.scratch/opus-video-handoff')}});
 for(const javaScriptEnabled of [true,false]){
  ...
  assert.ok((await page.locator('body').innerText()).includes('Refresh the old Opus session first'));
  let localLinks=0;
 ...
 result.result='PASS';result.packageMembersPreviouslyVerified=verification.zipMembersMatchSourceAndReceipt;
