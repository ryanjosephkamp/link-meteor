...
for(const c of verification.commits)assert.ok(md.includes(c.sha));
...
let browser;
try{
 browser=await chromium.launch({headless:true,env:{...process.env,TMPDIR:resolve(root,'.scratch/publication-proposal')}});
 for(const javaScriptEnabled of [true,false]){
  ...
  assert.ok((await page.locator('body').innerText()).includes('Original PR #1'));
  let localLinks=0;
 ...
 result.result='PASS';result.exactCommitCount=verification.commits.length;
