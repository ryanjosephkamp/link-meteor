...
import {createState,SETTINGS_DEFAULTS} from '../src/core/model.js';

const event=()=>({listeners:[],addListener(fn){this.listeners.push(fn);}});
const local={linkMeteorState:createState()},session={};
let failLocal=false,failSession=false,failTabCreate=false,permitted=true;
...
test('background adverse paths using explicit Chrome-API simulation',async t=>{
 ...
 await t.test('Review tab failure returns successful save and a separate warning',async()=>{failTabCreate=true;const before=local.linkMeteorState.collections[0].links.length;const reply=await new Promise(resolve=>chrome.runtime.onMessage.listeners[0]({type:'capture.commit',review:true,links:[{anchorText:'Saved once',url:'https://fixture.test/one'}]},{tab:tabs[0]},resolve));failTabCreate=false;assert.equal(reply.ok,true);assert.equal(reply.data.count,1);assert.match(reply.data.warning,/saved.*review tab/);assert.equal(local.linkMeteorState.collections[0].links.length,before+1);});
 await t.test('a stored 0.2.2 state reads with default settings and is upgraded by the next write',async()=>{const before=structuredClone(local.linkMeteorState);const old={...structuredClone(before),settings:{holdKey:'r',holdOrigins:[]}};local.linkMeteorState=structuredClone(old);const reply=await call({type:'state.get'});assert.equal(reply.ok,true);assert.deepEqual(reply.data.collections,old.collections);assert.deepEqual(reply.data.settings,{...SETTINGS_DEFAULTS,holdKey:'r',holdOrigins:[],holdExceptions:[]});assert.deepEqual(local.linkMeteorState,old,'reading does not write');const written=await call({type:'state.mutate',action:{type:'collection.create',name:'After upgrade'}});assert.equal(written.ok,true);assert.equal(local.linkMeteorState.settings.holdScope,'sites');assert.deepEqual(local.linkMeteorState.collections.slice(0,old.collections.length),old.collections);local.linkMeteorState=before;});
