...
try{
 ...
 await ui.bringToFront();await until(async()=>(await ui.locator('#capture-report').innerText()).includes('denied'),'Revoked capture report');await ui.screenshot({path:resolve(evidence,'permission-denied.png')});result.result='PASS';result.limits=['Native Deny button was not verified. Automated permission re-requests were granted by the test browser; actual missing/revoked access was verified separately.'];
