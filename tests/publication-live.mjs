// Live Pages QA adapted from tests/site-check.mjs at approved snapshot 6966200; only server/base/output/runtime paths changed.
...
const evidence = resolve(import.meta.dirname, '..', process.env.LINK_METEOR_EVIDENCE_DIR || 'artifacts/publication/2026-09-25');
...
const origin = 'https://ryanjosephkamp.github.io';
const server = {close(){}};
const base = `${origin}/link-meteor/`;
const browser = await playwright.chromium.launch({env:{...process.env,TMPDIR:resolve(import.meta.dirname, '../.scratch/publication-01')}});
const problems = [];
